...
import { 
  FaLock, FaShieldAlt, FaEye, FaEyeSlash, 
  FaExclamationCircle, FaArrowLeft, FaKey 
} from "react-icons/fa";
...
const ChangePasswordPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setIsLoading(true);
      const response = await axiosInstance.patch("/seller/update-password", {
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 p-4 md:p-10 flex flex-col items-center justify-center font-sans">
      ...
      >
        <Link href="/seller/dashboard" className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-slate-900 transition-colors mb-6 group">
        ...
        <div className="bg-white rounded-[3.5rem] p-8 md:p-12 border border-slate-100 shadow-2xl shadow-slate-200/60 relative overflow-hidden">
          ...
          <header className="mb-10 text-center">
            <div className="bg-teal-500 w-16 h-16 rounded-4xl flex items-center justify-center text-white mx-auto mb-6 shadow-lg shadow-teal-500/20">
              <FaKey size={24} />
            </div>
            <h1 className="text-2xl font-black text-slate-900 uppercase tracking-tight">Set New Password</h1>
            <p className="text-slate-400 text-[10px] font-bold mt-2 uppercase tracking-widest">Logged in as {authSeller.name}</p>
          </header>
                ...
                {[1, 2, 3, 4].map((step) => (
                  <div 
                    ...
                    className={`h-1 flex-1 rounded-full transition-colors ${
                      formData.newPassword.length >= step * 2 ? "bg-teal-500" : "bg-slate-100"
                    }`}
                  />
                ))}
              </div>
